...
class Embedding(nn.Module):
    ...
    def __init__(self, word_vec_mat, max_length, word_embedding_dim=50, pos_embedding_dim=5):
        ...
        unk = torch.zeros(1, word_embedding_dim)#生成1*word_embedding_dim维元素为0的矩阵
        blk = torch.zeros(1, word_embedding_dim)
        word_vec_mat = torch.from_numpy(word_vec_mat)#将数组转化为张量
        self.word_embedding = nn.Embedding(word_vec_mat.shape[0] + 2, self.word_embedding_dim, padding_idx=word_vec_mat.shape[0] + 1)#词与词向量的映射
